...
4. Meta Layer에 training_focus를 설정하여 이 문제를 통해 훈련하고자 하는 핵심 역량 목록을 함께 구조화합니다.
...
>   - problem_type, meta_layer, user_view_layer, system_view_layer, evaluation_layer
...
meta_layer 객체 내부 필드는 아래 7개만 사용합니다.
...
7. training_focus : 이 문제를 통해 훈련하고자 하는 핵심 역량 목록 (문자열 배열)
   - 형식: 문자열 배열. 예: `\"training_focus\": [\"Data_Literacy\", \"Problem_Structuring\", \"Strategic_Thinking\"]`
   - 각 값은 아래 정의된 사전 정의 역량 코드 중에서 선택합니다.
   - 최소 1개, 보통 2~5개 정도를 선택합니다.
   

[Meta Layer – training_focus 공통 역량 리스트]
- 형식: 문자열 배열 (예: [\"Data_Literacy\", \"Problem_Structuring\"])
- 표기 규칙: 영문 + PascalCase + 언더스코어 (예: Data_Literacy)

A. 분석/사고 계열
- Data_Literacy  
  : 주어진 텍스트·수치를 읽고 기본적인 지표 관계와 크기 감각을 이해하는 능력
- Problem_Structuring  
  : 복잡한 상황을 여러 하위 문제로 쪼개어 구조화하고, 무엇부터 다룰지 틀을 세우는 능력
- Hypothesis_Thinking  
  : 불완전한 정보 속에서 그럴듯한 가설을 세우고, 필요한 검증 포인트를 떠올리는 능력
- Root_Cause_Analysis  
  : 겉으로 드러난 결과 뒤에 숨은 근본 원인을 단계적으로 파고드는 능력
- Insight_Generation  
  : 데이터·텍스트에서 새로운 관점이나 의미 있는 발견을 뽑아내는 능력
- Strategic_Thinking  
  : 단일 액션이 아닌 중장기 방향성과 우선순위를 고려해 방향을 설계하는 능력
- Prioritization  
  : 여러 과제 중 임팩트·긴급도·실행 가능성을 기준으로 우선순위를 정하는 능력
- Scenario_Planning  
  : "만약 A라면 / B라면"과 같은 여러 시나리오를 두고 대응 전략을 준비하는 능력
- Decision_Criteria_Design  
  : 여러 선택지 중 무엇을 기준으로 고를지, 명확한 평가 기준을 설계하는 능력
- Tradeoff_Analysis  
  : 하나를 선택하면 다른 것을 포기해야 하는 상황에서 손익을 비교하고 타협점을 찾는 능력
- Constraint_Handling  
  : 예산·시간·리소스 등의 제약 안에서 현실적인 대안을 찾는 능력
- Value_Clarification  
  : 개인/팀/조직이 진짜 중요하게 여기는 가치를 분명히 하고 그 기준으로 결정하는 능력

B. 커뮤니케이션/표현 계열
- Stakeholder_Communication  
  : 이해관계자(경영진, 동료, 클라이언트 등)에 맞는 언어와 관점으로 설득·보고하는 능력
- Structured_Writing  
  : 글을 서론–본론–결론 또는 항목형 구조로 논리 정연하게 정리해서 쓰는 능력
- Quantitative_Reporting  
  : 차트 없이 텍스트와 숫자만으로도 지표를 명확하게 보고하는 능력
- Experience_Storytelling  
  : 자신의 경험을 상황–과제–행동–결과 구조로 설득력 있게 전달하는 능력
- Self_Reflection  
  : 자신의 강점·약점·실수와 배움을 스스로 돌아보고 언어화하는 능력

C. 실행/운영 계열
- Goal_Setting  
  : 막연한 바람이 아니라 구체적인 목표 상태·수치로 정의하는 능력
- Action_Planning  
  : 목표를 행동 단위로 쪼개어 무엇을 언제 어떻게 할지 계획하는 능력
- Progress_Tracking  
  : 계획 대비 실제 진행 상황을 주기적으로 체크하고 상태(완료/진행/지연)를 관리하는 능력
- Issue_Management  
  : 문제나 리스크 발생 시 원인·영향·조치 내역을 정리하고 후속 조치를 관리하는 능력
- Project_Scoping  
  : 프로젝트의 범위와 목표, 그리고 이번에 하지 않을 것까지 명확히 정의하는 능력
- Success_Metrics_Design  
  : 성공/실패를 판단할 핵심 지표(KPI)를 설계하는 능력
- Habit_Design  
  : 일상 루틴·습관을 작은 단위로 설계하고 꾸준히 유지할 수 있게 만드는 능력
- Time_Management  
  : 제한된 시간 안에서 중요한 일에 시간을 재배분하고 관리하는 능력
- Self_Management  
  : 개인의 에너지·감정·집중력을 관리하며 계획한 실행을 유지하는 능력

> **중요:**  
> `meta_layer` 안에는 위 7개 필드 외의 **새로운 필드(예: created_by, lang 등)를 생성하지 않습니다.**  
...
7. `opening_line` : AI가 대화를 시작할 때 던지는 첫 문장 (화두/시작 힌트)
- 역할:
  - 사용자가 문제를 어떻게 시작해야 할지 막막하지 않도록,
    **"어디서부터 볼지 / 무엇을 먼저 물어볼지"에 대한 방향만 제시하는 한 문장**입니다.
  - **핵심 문제의 정답이나 해법을 직접적으로 말하지 않고**,  
    문제를 풀어가는 출발점(첫 관찰 지점, 첫 질문 포인트)을 잡아주는 "힌트"여야 합니다.

- 형태 예시:
  - `\"안녕하세요. 최근 신규 고객 지표가 이상합니다. 먼저 어떤 데이터를 확인하시겠습니까?\"`
  - `\"안녕하세요. 이번 캠페인 결과가 기대에 못 미쳤습니다. 먼저 어떤 지표를 함께 살펴보면 좋을까요?\"`
  - `\"안녕하세요. 오늘은 의사결정을 도와드릴게요. 우선 어떤 선택지들을 후보로 두고 계신지부터 알려주시겠어요?\"`

- 작성 규칙:
  - 존댓말 1문장 또는 2문장 이내로 작성합니다.
  - "먼저 ~해볼까요?", "어떤 ~부터 확인하시겠어요?"처럼 **질문형 마무리**를 권장합니다.
  - 문제의 전체 맥락은 간단히 짚되,  
    **해결 방향이나 구체적인 전략·답을 직접 제시하지 않습니다.**
...
────────────────────────────────────
3.problem_type & target_template_code & training_focus 설정
...
3-3. training_focus 설정 규칙

각 문제는 선택된 `problem_type`/`target_template_code`에 따라  
**대표적인 핵심 역량 세트(training_focus)**를 자동으로 설정합니다.  
필요 시 입력 문제의 맥락을 보고 1~2개 정도를 추가/교체할 수 있습니다.

- **T1. Analysis Report – problem_type = \"Analysis Report\"**  
  - 기본 추천:
    ```json
    [\"Data_Literacy\", \"Problem_Structuring\", \"Hypothesis_Thinking\", \"Root_Cause_Analysis\", \"Insight_Generation\", \"Structured_Writing\"]
    ```

- **T2. Strategy Proposal – problem_type = \"Strategy Proposal\"**  
  - 기본 추천:
    ```json
    [\"Problem_Structuring\", \"Strategic_Thinking\", \"Prioritization\", \"Scenario_Planning\", \"Risk_Management\", \"Stakeholder_Communication\"]
    ```

- **T3. Interview Response – problem_type = \"Interview Response\"**  
  - 기본 추천:
    ```json
    [\"Problem_Structuring\", \"Experience_Storytelling\", \"Self_Reflection\", \"Stakeholder_Communication\", \"Structured_Writing\"]
    ```

- **T4. Work Report – problem_type = \"Work Report\"**  
  - 기본 추천:
    ```json
    [\"Quantitative_Reporting\", \"Structured_Writing\", \"Progress_Tracking\", \"Issue_Management\", \"Stakeholder_Communication\", \"Time_Management\"]
    ```

- **T5. Project Kickoff Brief – problem_type = \"Project Kickoff Brief\"**  
  - 기본 추천:
    ```json
    [\"Project_Scoping\", \"Goal_Setting\", \"Success_Metrics_Design\", \"Scenario_Planning\", \"Stakeholder_Communication\", \"Action_Planning\"]
    ```

- **T6. Decision Sheet – problem_type = \"Decision Sheet\"**  
  - 기본 추천 (Constraint_Handling 대신 Conditions_Handling을 사용해도 됨):
    ```json
    [\"Conditions_Handling\", \"Decision_Criteria_Design\", \"Tradeoff_Analysis\", \"Value_Clarification\", \"Risk_Management\"]
    ```

- **T7. Simple Plan & Checklist – problem_type = \"Simple Plan & Checklist\"**  
  - 기본 추천:
    ```json
    [\"Goal_Setting\", \"Action_Planning\", \"Habit_Design\", \"Time_Management\", \"Self_Management\", \"Progress_Tracking\"]
    ```

> 원칙:  
> - `training_focus`는 반드시 **위에서 정의한 코드들만** 사용합니다.  
> - 한 문제당 2~5개 정도를 선택하는 것을 권장합니다.  
> - 문제의 시나리오가 특정 역량을 강하게 요구할 때는, 그에 맞게 1~2개를 교체/추가해도 됩니다.
...
> ⚠️ `title`, `task`, `topic_summary`, `topic` 등은 **입력에서만 사용**하고,  
> **최종 출력에는 `meta_layer` / `user_view_layer` 안의 필드로만 반영합니다.**  
> 루트 레벨에 동일 이름의 필드를 다시 만들지 않습니다.
...
3. **새로 추가/보강할 수 있는 필드는 다음으로 제한합니다.**
   ...
   - 각 레이어 내부:
     - meta_layer: `id`, `category`, `topic`, `difficulty`, `target_template_code`, `time_limit`, `training_focus`
